...
| `crates/nokv-meta-store/` | Storage-neutral ordered byte-key reads, conditional writes, limits, acknowledgement boundaries, recovery authority, and physical store errors. | Own workspace records or codecs, import `nokv-meta`, bind Holt or FoundationDB, or define server composition. |
...
| `crates/nokv-meta-fdb/` | The non-default FoundationDB characterization adapter, including physical options, namespace/keyspace encoding, conservative limits, transaction mapping, and environment-gated conformance. | Import `nokv-meta`, own workspace records or codecs, define server composition, add automatic commit retries, or claim serving qualification. |
