...
| `crates/nokv-agent/` | Transport-free agent surfaces: the NoKV namespace tool dispatcher, model-facing result shaping, LingTai-derived event-index types/query contracts, and the local agent-index CLI. | Depend on `nokv-client`, `nokv-protocol`, or `nokv-control`; own NoKV namespace metadata semantics; treat LingTai JSONL-derived indexes as source of truth; hide benchmark-only behavior in product APIs. |
