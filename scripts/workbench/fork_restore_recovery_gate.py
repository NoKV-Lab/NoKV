...
# Every restore publishes exactly two destination-owned manifest objects
...
# Stable v1 magic of the fixed 48-byte workspace RPC handshake payload that
# precedes the operation frame on every client connection
# (crates/nokv-protocol/src/handshake.rs).
WORKSPACE_HANDSHAKE_MAGIC = b"NOKVHS1\0"
...
def is_handshake_frame(payload: bytes) -> bool:
    return payload.startswith(WORKSPACE_HANDSHAKE_MAGIC)
...
class ResponseDropProxy:
    ...
    def _forward(self, client: socket.socket) -> None:
        # One client connection carries the fixed workspace handshake frame
        # followed by the operation frame; every frame is length-prefixed and
        # answered by exactly one owner frame, so the relay stays frame-generic
        # and only classifies non-handshake requests for the armed drop.
        try:
            ...
            with socket.create_connection(target, timeout=self.timeout) as owner:
                ...
                while True:
                    ...
                    if not is_handshake_frame(request):
                        with self._lock:
...
def download_json_object(
    ...
    *,
    ...
    downloaded = evidence.root / f"{label}.json"
    run(
        aws_command(
            aws,
            endpoint,
    ...
    try:
        ...
        raise WorkflowFailure(f"{label}: object {key} is not a JSON document") from error


def classify_manifest_objects(
    manifests: dict[str, object], label: str
) -> tuple[str, str]:
    """Split the two added restore objects into (restore manifest, run manifest)."""
    by_schema: dict[str, str] = {}
    for key, manifest in manifests.items():
        schema = manifest.get("schema") if isinstance(manifest, dict) else None
        if schema not in {RESTORE_MANIFEST_SCHEMA, RUN_MANIFEST_SCHEMA} or schema in by_schema:
            raise WorkflowFailure(
                f"{label} added objects are not one restore manifest plus one run manifest: "
                f"{sorted(manifests)}"
            )
        by_schema[schema] = key
    if len(by_schema) != RESTORE_MANIFEST_OBJECTS:
        raise WorkflowFailure(f"{label} lacks one of the two destination manifests")
    return by_schema[RESTORE_MANIFEST_SCHEMA], by_schema[RUN_MANIFEST_SCHEMA]
...
def manifest_only_delta(
    ...
    *,
    ...
    if overwritten or len(added) != RESTORE_MANIFEST_OBJECTS:
        raise WorkflowFailure(
            f"{label} was not zero-copy: overwritten={overwritten}, added={added}"
        )
    manifests = {
        key: download_json_object(
            key,
            aws=aws,
            endpoint=endpoint,
            bucket=bucket,
            ...
            timeout=timeout,
            evidence=evidence,
            label=f"{label}-download-manifest-{index}",
        )
        for index, key in enumerate(added)
    }
    restore_key, run_key = classify_manifest_objects(manifests, label)
    validate_restore_manifest(manifests[restore_key], source, destination, label)
    validate_run_manifest(manifests[run_key], destination, f"{label} run manifest")
    return {"restore_manifest_object": restore_key, "run_manifest_object": run_key}


def validate_run_manifest(
    manifest: object, workbench: str, label: str = "run manifest"
) -> None:
    if not isinstance(manifest, dict):
        raise WorkflowFailure(f"{label} is not a JSON object")
    commit_identity = manifest.get("commit_identity")
    if (
        manifest.get("schema") != RUN_MANIFEST_SCHEMA
        or manifest.get("workbench_id") != workbench
        or manifest.get("workbench_path") != f"{WORKBENCH_ROOT}/{workbench}"
        or not isinstance(commit_identity, str)
        or len(commit_identity) != 64
    ):
        raise WorkflowFailure(f"{label} is not owned by workbench {workbench}")
...
def validate_restore_manifest(
    ...
    source_path = f"{WORKBENCH_ROOT}/{source}"
...
def validate_gate_evidence(evidence: dict[str, object]) -> None:
    ...
    for name, value in (
        ...
        if (
            ...
            or value.get("manifest_objects_added") != RESTORE_MANIFEST_OBJECTS
        ):
            raise WorkflowFailure(
                f"{name} did not add exactly two destination-owned manifests"
            )
    ...
    if (
        ...
        or nested.get("source_recommit_replace") is not True
...
def execute(args: argparse.Namespace) -> dict[str, object]:
    ...
    if args.lease_ttl_seconds <= 0:
        raise WorkflowFailure("--lease-ttl-seconds must be positive")
    ...
    # Durable RootId-to-AgentId binding: provision and every Agent-facing
    # client command carry it; the shard owner (`serve`) must not.
    ...
    # Payload blocks and both destination manifests live in the shard/root
    # artifact keyspace. The owner also publishes metadata recovery-log
    # segments under nokv/recovery/ before acknowledging every write; those
    # are durability objects, not payload copies, so the zero-copy inventory
    # is scoped to the artifact keyspace.
    artifact_prefix = f"{object_root}/nokv/artifacts/{shard_id}/{root_id}"
    ...
    try:
        ...
        # The successor owner reopens the Holt store, reconciles and publishes
        # recovery state, and only then renews its lease for the first time
        # (crates/nokv-server/src/bootstrap.rs). That bootstrap must fit in one
        # etcd lease TTL, so the gate uses the product default rather than the
        # two-second fault-injection TTL of the sibling gates.
        common[common.index("--etcd-lease-ttl-seconds") + 1] = str(args.lease_ttl_seconds)
        ...
        # A restored destination is already committed (restore materializes
        # its run manifest and installs a commit head), so proving that the
        # fork is independently recommittable requires an explicit replace.
        workbench_call(
            client,
            "workbench_commit",
            {
        ...
        validate_run_manifest(
            download_json_object(
                nested_source_commit_objects[0],
                aws=aws,
                endpoint=s3_endpoint,
                bucket=bucket,
                environment=aws_env,
                ...
                timeout=args.timeout,
                evidence=evidence,
                label="nested-source-commit-download-manifest",
            ),
            concurrent_destination,
            "recommitted fork run manifest",
        )
        ...
        result: dict[str, object] = {
            ...
            "object_inventory": {
                ...
                "excludes": "nokv/recovery/ metadata recovery-log segments",
            },
            "restarts": restarts,
            "forks": {
                "prepare_ack_loss": {
        ...
        # Persist the complete terminal record before contract validation so a
        # failed contract still leaves the per-scenario numbers on disk.
        evidence.json("terminal-evidence.json", result)
